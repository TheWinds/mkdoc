...
func scanSchemas(project *mkdoc.Project, filterTag string) ([]*schema.Schema, error) {
	...
	for _, scanner := range project.Scanners {
		log.Printf("🔎  scan doc annotations (use %s)\n", scanner.Name())
...
func makeDoc() error {
	...
	if len(apiDefs) == 0 {
		log.Printf("👽  no api is matched,all tags:\n")
		// TODO: show all tags
	...
	for n, def := range apiDefs {
		log.Printf("\r🔥 parse & build api '%s' [%d/%d]          ", def.Name, n+1, len(apiDefs))
...
func writeFile(dir, name string, data []byte) error {
	...
	log.Printf("📖  write api doc to './docs/%s/%s'\n", dir, name)
